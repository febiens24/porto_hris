...
import 'bloc/leave_approval_history_bloc.dart';

class LeaveApprovalHistoryPage extends StatelessWidget {
  const LeaveApprovalHistoryPage({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => LeaveApprovalHistoryBloc(
        getLeaveApprovalHistoryList: injector(),
      )..add(const LoadLeaveApprovalHistoryList()),
      child: const LeaveApprovalHistoryPageView(),
    );
  }
}

class LeaveApprovalHistoryPageView extends StatefulWidget {
  const LeaveApprovalHistoryPageView({Key? key}) : super(key: key);

  @override
  State<LeaveApprovalHistoryPageView> createState() =>
      _LeaveApprovalHistoryPageViewState();
}

class _LeaveApprovalHistoryPageViewState
    extends State<LeaveApprovalHistoryPageView> {
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          BlocBuilder<LeaveApprovalHistoryBloc, LeaveApprovalHistoryState>(
                                      ...
                                      Expanded(
                                        child: ListView.builder(
                                          ...
                                          itemBuilder: (context, index) {
                                            final data = state.listTypes[index];
                                            return index == 0
          ...
          BlocBuilder<LeaveApprovalHistoryBloc, LeaveApprovalHistoryState>(
            builder: (context, state) {
              switch (state.status) {
                case LeaveApprovalHistoryStatus.initial:
                  return const Center(child: CircularProgressIndicator());
                case LeaveApprovalHistoryStatus.success:
                  final leaveDatas = state.leaves;
                  if (leaveDatas.isEmpty) {
                    return const Expanded(child: EmptyWidget());
                  } else {
                    return Expanded(
                      child: ListView.separated(
                        itemCount: leaveDatas.length,
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          ...
                          return ListTile(
                            ...
                            leading: CachedNetworkImage(
                              ...
                              fit: BoxFit.contain,
                              imageUrl: leave.leaveRequesterProfilePicture!,
                            ...
                            title: SizedBox(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    '${leave.leaveRequesterName} - ${leave.leaveRequesterRole}',
                            ...
                            subtitle: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  '${leave.leaveDateFrom} - ${leave.leaveDateTo}',
                                  style: textTheme.bodySmall,
                                )
                              ],
                            ),
                ...
                case LeaveApprovalHistoryStatus.failure:
