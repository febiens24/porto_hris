...
import '../request_dispensation/request_dispensation_page.dart';
import 'bloc/dispensation_list_bloc.dart';

class DispensationPage extends StatelessWidget {
  const DispensationPage({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocProvider<DispensationListBloc>(
      create: (context) => DispensationListBloc(
        getDispensationList: injector(),
      )..add(const LoadDispensationList()),
      child: const DispensationPageView(),
    );
  }
}

class DispensationPageView extends StatefulWidget {
  const DispensationPageView({Key? key}) : super(key: key);

  @override
  State<DispensationPageView> createState() => _DispensationPageViewState();
}

class _DispensationPageViewState extends State<DispensationPageView> {
  ...
  void _onScroll() {
    if (_isBottom) {
      context.read<DispensationListBloc>().add(const LoadDispensationList());
    }
  }
      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            //   title: 'Dispensation',
            // ),
            BlocBuilder<DispensationListBloc, DispensationListState>(
              builder: (context, state) {
                switch (state.status) {
                  case DispensationListStatus.initial:
                    return const SizedBox.shrink();
                  case DispensationListStatus.success:
                    return WidgetToolBar(
                      title: 'Dispensation',
                      onSubmitted: (query) {
                        // print(query);
                        BlocProvider.of<DispensationListBloc>(context).add(
                  ...
                  case DispensationListStatus.failure:
                    return const SizedBox.shrink();
                }
              },
            ),
            SizedBox(
              height: 50,
              child: BlocBuilder<DispensationListBloc, DispensationListState>(
                builder: (context, state) {
                  switch (state.status) {
                    case DispensationListStatus.initial:
            ...
            BlocBuilder<DispensationListBloc, DispensationListState>(
              builder: (context, state) {
                switch (state.status) {
                  ...
                  case DispensationListStatus.success:
                    if (state.dispensations.isEmpty) {
                      return const Center(child: Text('no posts'));
                    }
                    final dispensations = state.dispensations;
                    return Flexible(
                      child: ListView.separated(
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          final dispensationData = dispensations[index];
                          return Material(
                            child: InkWell(
                              onTap: () {
                                Navigator.push(
                                  ...
                                  MaterialPageRoute(
                                    builder: (context) =>
                                      ...
                                      dispensationId:
                                          dispensationData.dispensationId,
                                    ),
                                  ),
                                );
                              },
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => const RequestDispensationPage(),
            ),
          );
        },
