...
import 'bloc/dispensation_detail_bloc.dart';

class DispensationDetailPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) =>
          DispensationDetailBloc(getDispensationDetail: injector())
            ..add(
              LoadDispensationDetail(dispensationId),
            ),
      child: DispensationDetailPageView(
        source: source,
      ),
    );
  }
...
class DispensationDetailPageView extends StatefulWidget {
  final String? source;
  const DispensationDetailPageView({
    Key? key,
    this.source,
  }) : super(key: key);

  @override
  State<DispensationDetailPageView> createState() =>
      _DispensationDetailPageViewState();
}

class _DispensationDetailPageViewState
    extends State<DispensationDetailPageView> {
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar:
          BlocBuilder<DispensationDetailBloc, DispensationDetailState>(
        builder: (context, state) {
          if (state is DispensationDetailLoadSuccess) {
            final data = state.dispensationDetail.result;
            if (data.dispensationStatus != "waiting") {
              return const BottomAppBar();
            } else if (data.dispensationStatus == "waiting" &&
                widget.source != null &&
                    ...
                    children: <Widget>[
                      OutlinedButton(
                        ...
                        onPressed: () {
                          ...
                          _onRejectButtonPressed(context, data.dispensationId);
                        },
                      ...
                      ElevatedButton(
                        ...
                        onPressed: () {
                          // Respond to button press
                          _onApproveButtonPressed(context, data.dispensationId);
                        },
                  ...
                  child: Row(
                    ...
                    children: <Widget>[
                      ElevatedButton(
                        ...
                        onPressed: () {
                          // Respond to button press
                          _onCancelButtonPressed(context, data.dispensationId);
                        },
          ...
          children: [
            Padding(
              ...
              child: Column(
                children: [
                  widget.source == "approval"
                      ? BlocBuilder<DispensationDetailBloc,
                          DispensationDetailState>(
                          builder: (context, state) {
                            if (state is DispensationDetailLoadSuccess) {
                              final data = state.dispensationDetail.result;
                              // return ListTile(
                              ...
                              return Row(
                                children: [
                                  ...
                                  Column(
                                    ...
                                    children: [
                                      Text(
                                        data.dispensationRequesterName,
                                        style: textTheme.subtitle1,
                                      ),
                                      Text(
                                        data.dispensationRequesterRole,
                    ...
                    builder: (context, state) {
                      if (state is DispensationDetailLoadSuccess) {
                        return DataWidget(
                          textTheme: textTheme,
                          data: ReadMoreText(
                            state.dispensationDetail.result
                          ...
                          label: 'Nomor Dokumen',
                          status: state
                              .dispensationDetail.result.dispensationStatus,
                        );
                      } else {
                        return DataWidget(
                          textTheme: textTheme,
                    ...
                    builder: (context, state) {
                      if (state is DispensationDetailLoadSuccess) {
                        return DataWidget(
                          textTheme: textTheme,
                          data: ReadMoreText(
                            state.dispensationDetail.result.dispensationType,
                            trimLines: 2,
                            colorClickableText: const Color(0xff6792BA),
                            trimMode: TrimMode.Line,
                            trimCollapsedText: 'Show more',
                    ...
                    builder: (context, state) {
                      if (state is DispensationDetailLoadSuccess) {
                        return DataWidget(
                          textTheme: textTheme,
                          data: ReadMoreText(
                            state.dispensationDetail.result.dispensationDate,
                            trimLines: 2,
                            colorClickableText: const Color(0xff6792BA),
                            trimMode: TrimMode.Line,
                            trimCollapsedText: 'Show more',
                    ...
                    builder: (context, state) {
                      if (state is DispensationDetailLoadSuccess) {
                        return DataWidget(
                          textTheme: textTheme,
                          data: ReadMoreText(
                            state.dispensationDetail.result.dispensationNotes,
                            trimLines: 2,
                            colorClickableText: const Color(0xff6792BA),
                            trimMode: TrimMode.Line,
                            trimCollapsedText: 'Show more',
                              ...
                              Column(
                                ...
                                children: [
                                  ...
                                  BlocBuilder<DispensationDetailBloc,
                                    ...
                                    builder: (context, state) {
                                      if (state
                                          is DispensationDetailLoadSuccess) {
                                        if (state
                                            .dispensationDetail
                                            .result
                                          ...
                                          return Row(
                                            ...
                                            crossAxisAlignment:
                                                CrossAxisAlignment.start,
                                            children: state.dispensationDetail
                                                .result.dispensationAttachments
                                                .map((i) {
                                              return InkWell(
                                                onTap: () {
                                                  print('asd');
            ...
            Padding(
              padding: const EdgeInsets.all(20.0),
              child: SizedBox(
                child: BlocBuilder<DispensationDetailBloc,
                    DispensationDetailState>(
                  builder: (context, state) {
                    if (state is DispensationDetailLoadSuccess) {
                      return Column(
                        ...
                        children: [
                          ...
                          ...state
                              .dispensationDetail.result.dispensationApprovers
                              .map(
                            (e) {
